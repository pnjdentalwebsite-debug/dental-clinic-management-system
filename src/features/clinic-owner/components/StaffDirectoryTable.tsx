...
export function StaffDirectoryTable({
  ...
  const isAllSelected = staffList.length > 0 && staffList.every((s) => selectedIds.includes(s.id));
  const isPartiallySelected = selectedIds.length > 0 && !isAllSelected;

  const totalPages = Math.max(1, Math.ceil((totalCount || staffList.length) / pageSize));
  const startIndex = (currentPage - 1) * pageSize + 1;
  const endIndex = Math.min(currentPage * pageSize, totalCount || staffList.length);
            ...
            >
              <th style={{ padding: '1rem', width: '40px' }}>
                <input
                  ...
                  aria-label="Select all staff members on this page"
                />
              </th>
              <th style={{ padding: '1rem', minWidth: '220px' }}>Staff Member</th>
              <th style={{ padding: '1rem', minWidth: '150px' }}>Role / Designation</th>
              <th style={{ padding: '1rem', minWidth: '160px' }}>Contact Number</th>
              <th style={{ padding: '1rem', minWidth: '150px' }}>Email Address</th>
            ...
            {staffList.length > 0 ? (
              staffList.map((staff) => (
                <StaffTableRow
                  ...
                  isChecked={selectedIds.includes(staff.id)}
                  onSelect={() => onSelectStaff(staff.id)}
                  onToggleCheck={() => onToggleSelectId?.(staff.id, !selectedIds.includes(staff.id))}
                  onAction={onAction}
                />
              ))
            ) : (
              <tr>
                <td colSpan={7} style={{ padding: '4rem 1.25rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
                  <div style={{ display: 'grid', gap: '0.4rem', justifyItems: 'center' }}>
                    <strong style={{ fontSize: '1rem', color: 'var(--text-primary)' }}>No staff members found</strong>
                    <span style={{ fontSize: '0.84rem' }}>Try adjusting your search query or status filter.</span>
                  </div>
                </td>
              </tr>
            )}
          ...
          <div style={{ color: 'var(--text-muted)' }}>
            {totalCount > 0 ? (
              <span>
                ...
                <strong style={{ color: 'var(--text-primary)' }}>{totalCount}</strong> staff members
              </span>
            ) : (
