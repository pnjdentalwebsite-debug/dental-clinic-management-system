...
import { ClinicLaboratoryRow } from './ClinicLaboratoryRow';
...
export function ClinicLaboratoryTable({
  labs,
  selectedLabId,
  onSelectLab,
  onAction,
  ...
  const isAllSelected = labs.length > 0 && selectedIds.length === labs.length;
  const isSomeSelected = selectedIds.length > 0 && !isAllSelected;
  const totalPages = Math.ceil((totalCount || labs.length) / pageSize) || 1;
  ...
  return (
    ...
    >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            <span style={{ fontSize: '0.82rem', fontWeight: 700, color: '#0284c7' }}>
              {selectedIds.length} laborator{selectedIds.length > 1 ? 'ies' : 'y'} selected
            </span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            {onBulkActivate && (
              ...
              >
                <Power size={13} /> Connect Selected
              </button>
            )}
            {onBulkDeactivate && (
              <button
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
          <thead>
            <tr style={{ backgroundColor: 'var(--background)', borderBottom: '1px solid var(--border)', color: 'var(--text-secondary)', fontWeight: 600 }}>
              ...
              <th style={{ padding: '0.9rem 1rem' }}>Laboratory Name</th>
              ...
              <th style={{ padding: '0.9rem 1rem' }}>Turnaround Time</th>
            ...
            {labs.length === 0 ? (
              <tr>
                <td colSpan={8} style={{ padding: '3.5rem 1rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                  <div style={{ display: 'grid', gap: '0.35rem', justifyItems: 'center' }}>
                    <strong style={{ fontSize: '0.92rem', color: 'var(--text-primary)' }}>No dental laboratories found</strong>
              ...
              labs.map((lab) => (
                <ClinicLaboratoryRow
                  ...
                  isChecked={selectedIds.includes(lab.id)}
                  onToggleCheck={() => onToggleSelectId?.(lab.id)}
                  onAction={onAction}
                />
              ))
            )}
      ...
      >
        <span>
          Showing {labs.length > 0 ? (currentPage - 1) * pageSize + 1 : 0} to{' '}
          {Math.min(currentPage * pageSize, totalCount || labs.length)} of {totalCount || labs.length} laboratories
        </span>
