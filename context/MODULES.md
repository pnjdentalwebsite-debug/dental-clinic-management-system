...
- Second validation repair binds action-menu and side-preview View/Edit to the exact selected row UUID; existing-branch lifecycle controls are disabled/read-only, while create-form Save Draft remains supported.
- Zero-hours legacy behavior is regression-tested but not recreated in the linked tenant because every current clinic has seven persisted hour rows. **PHASE 2E.3B.2B = SECOND LIVE BROWSER VALIDATION BLOCKERS FIXED LOCALLY / LIVE BROWSER RE-VALIDATION REQUIRED.** Lifecycle mutations remain deferred.
