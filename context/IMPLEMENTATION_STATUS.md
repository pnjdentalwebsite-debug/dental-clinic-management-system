...
61. **Phase 2E.1 Clinic Owner First-Login Frontend Auth Cutover (August 30, 2026)**:
    - The Clinic Owner Login and mandatory first-login password flow now use the configured Supabase browser client, `auth.signInWithPassword`, authenticated `get_my_first_login_state()`, and the deployed `complete-initial-password` function.
    - The client uses the server-returned membership state as routing authority: zero or multiple active Clinic Owner memberships fail safely; `mustChangePassword=true` permits only Change Password and logout; after a successful completion it refreshes the authoritative state before opening Clinic Owner routing.
...
62. **Phase 2E.2 Platform Admin Real-Data Cutover — Remote Read Function Deployed; Live Validation Defect Fix Pending Redeploy/Revalidation (August 30, 2026)**:
    ...
    - Correctness follow-up removed the browser-wide directory snapshot: sign-in loads only aggregate Dashboard summary plus pending review data, while each list sends its current bounded page/search/filter contract and each detail requests one exact UUID. Users and Subscriptions now constrain the server dataset before pagination/counting; subscriber clinic/laboratory/associate/staff counts include only active records where status authority exists.
    ...
    - No migration is required. Local Edge Runtime bundling passed and an anonymous request returned `401`; 11 focused files / 79 tests and `npm run build` passed.
    - The first remote deployment and live browser pass confirmed real Dashboard, subscriber, clinic, payment, plan, and authentication data, then exposed navigation-order projection defects. The local correction now renders each list from its own bounded response, moves Dashboard/Subscriber global KPIs to the summary DTO, includes safe owner summaries for subscribers/clinics/subscriptions, and hides unsupported Platform mock reset/prototype/purge authority.
    - Status remains **REMOTE READ FUNCTION DEPLOYED / LIVE VALIDATION IN PROGRESS / DEFECT FIX PENDING REDEPLOY/REVALIDATION**. Phase 2E.2 is not closed.

63. **Phase 2E.2 Final Detail-Page Real-Data Coherence Patch (August 30, 2026)**:
    - The second live browser pass validated Platform Admin authentication, Dashboard, all six bounded list resources, logout, and direct subscriber routing. It exposed one remaining direct-refresh Subscriber Details defect: related owner, facility, plan-price, and paid-total values were being resolved from unrelated page caches or local defaults.
    - Exact subscriber detail DTOs now carry safe owner identity, current subscription pricing, active facilities/personnel, payment summaries, and server-computed financial totals. Subscriber Details uses those fields directly: owner identity never falls back to the business name, tier rate comes from `public.plans`, active clinic count is server-derived, and paid total is summed from approved real payments.
...
66. **Phase 2E.3A Authenticated Clinic Owner Tenant/Read Foundation (August 30, 2026)**:
    - Added an RLS-backed Clinic Owner bootstrap adapter and provider for authenticated owner identity, subscriber, current subscription, plan catalog values, normalized quotas, tenant clinics, and active clinic/laboratory/associate/staff usage.
    - The Clinic Owner shell now displays authoritative owner, organization, and plan identity. It shows controlled loading/error states with no mock fallback, and no longer exposes the development-only Reset Mock Data control.
    - Live browser validation confirmed the existing provisioned owner authenticates; the shell resolves Angelo Mhyr Lagsac, Angelo Dental Clinic, and Plus from the real active `clinic_owner` membership. RLS-scoped membership, profile, subscriber, subscription, plan, clinic, and resource reads succeed, including after a direct `/clinic/dashboard` refresh, with no visible runtime/RLS failure.
    - Logout clears Clinic Owner protected access and provider data; refreshing or reopening `/clinic/dashboard` does not restore the former session/scope. No branch, personnel, laboratory, subscription, or settings mutation was enabled.
    - The legacy Dashboard welcome email text, blank Clinic field, generic Subscription label, counts, branch overview, financial summary, and recent activity remain deferred and non-authoritative until Phase 2E.3B. Legacy `tenantScope.ts` and other mock-backed owner business pages remain outside this subphase.
    - **PHASE 2E.3A = COMPLETE / FRONTEND-RLS READ FOUNDATION / LIVE BROWSER VALIDATED / READY FOR CHECKPOINT.** No migration, Edge Function/RPC, deployment, merge, or Phase 2E.3B work was performed during closure.

67. **Phase 2E.3B.1 Clinic Owner Dashboard + Branch Directory Real-Data Read Cutover (August 31, 2026)**:
    - Dashboard identity now renders the provider's real owner display name, organization, and plan. Active clinic, associate, and staff KPIs use the RLS bootstrap resource counts; Patients remains explicitly unavailable pending its own cutover.
    - Clinic Branch Overview and `/clinic/branches` render only the provider's tenant clinics, including UUID-backed records, clinic number, main/satellite/primary state, status, address, contact, email, and real creation timestamp. Loading/backend failure remains distinct from a real empty clinic dataset.
    - Clinical financial totals and setup progress no longer display fabricated zeroes or percentages. Recent Activity now uses a subscriber-filtered, owner-RLS-visible `audit_events` read and has a truthful empty state without synthetic activity.
    - Clinic quota displays active usage against the normalized plan limit. All branch writes and mock-backed detail/workspace destinations are controlled unavailable until Phase 2E.3B.2; the Clinic Owner Prototype Mode badge is removed.
    - Focused 2E.3B.1 tests and production build pass. The wider Phase 2E regression run passed 102/103 tests; the sole failure is an unchanged Phase 2C.2A line-ending-sensitive `supabase/config.toml` assertion outside this scope.
    - Live browser validation passed: Angelo Mhyr Lagsac / Angelo Dental Clinic / Plus render authoritatively; Prototype Mode is absent; active clinic/associate/staff values are 1/0/0; Patients is controlled unavailable; real primary-clinic fields, financial deferral, and RLS-visible audit activity all render correctly through hard refresh.
    - The real branch directory displays Angelo Dental Clinic with truthful Primary/Active state, location, contact, email, and catalog-driven `1 / 3` clinic usage; no mock clinic rows appear and hard refresh retains the provider directory.
    - Add, Edit, and status mutation actions remain controlled read-only with no mock persistence. Logout clears protected routes/provider state, and direct Dashboard/Branches reopening returns to login.
    - **PHASE 2E.3B.1 = COMPLETE / REAL-DATA READ CUTOVER / LIVE BROWSER VALIDATED / READY FOR CHECKPOINT.** No migration, RPC, Edge Function, deployment, merge, or Phase 2E.3B.2 work was performed. Real branch create/edit/activate/deactivate/delete/set-primary actions and server-enforced clinic-quota transactions remain deferred.

68. **Phase 2E.3B.2A Clinic Branch Mutation Backend Contract (August 31, 2026)**:
    - Added one additive migration with authenticated-only `create_my_clinic_branch(jsonb)` and `update_my_clinic_branch(uuid, jsonb)` SECURITY DEFINER RPCs plus private caller/input/quota/DTO helpers.
    - Caller identity, subscriber, actor, subscription, plan, quota, clinic identity, status, and primary state are server-derived. Create serializes on the subscriber row, enforces the current plan's clinic quota, creates a non-primary UUID/`CLN-XXXXXXXXXX` branch, replaces all seven hours, and audits atomically. Update uses the same tenant lock, preserves protected identity/lifecycle fields, and atomically replaces hours when supplied.
    - Owner direct writes to `clinics` and `clinic_business_hours` are denied while owner SELECT and existing Platform Admin/service-role behavior are preserved. Lifecycle operations (activate/deactivate/set-primary/archive/restore) remain unimplemented.
    - Local pgTAP coverage, genuine two-connection quota concurrency validation, clean shadow migration application/schema diff, schema lint, security-advisor error gate, and production build pass. The broad Vitest command remains noisy for pre-existing Deno/config/mock-suite and worker-startup issues outside this database-only change.
    - Migration `20260831011234` is deployed as the latest linked-development migration. Remote transaction-scoped validation passed for function/helper privileges, owner/Platform policy behavior, create/update, identity/hours/audit authority, quota variants, cross-tenant safe not-found, stable errors, and zero fixture residue. The real Angelo tenant remains one active primary clinic on Plus at `1 / 3` through authenticated RLS reads.
    - Remote schema lint reports no errors. Security advisors report no error; the new warnings identify the two intentionally authenticated SECURITY DEFINER RPC boundaries, while other findings are pre-existing project advisories. Remote concurrency and rollback injection were skipped for safety and remain covered by the passed local tests.
    - **PHASE 2E.3B.2A = BACKEND CONTRACT REMOTELY DEPLOYED / REMOTE CONTRACT VALIDATED / FRONTEND NOT WIRED / READY FOR CHECKPOINT.** Phase 2E.3B.2 remains incomplete.

69. **Phase 2E.3B.2B Clinic Owner Add/Edit Branch Frontend Real-Data Cutover (August 31, 2026)**:
    - Added typed frontend adapters for deployed `create_my_clinic_branch` and `update_my_clinic_branch`, with a strict editable-field allowlist, safe DTO/error mapping, and explicit seven-day business-hours conversion. The browser never submits subscriber, clinic number, status, primary, team, plan, quota, or audit authority.
    - Add/View/Edit now retain the existing route and stepper architecture. Detail is read by exact clinic UUID through owner RLS from `clinics` plus `clinic_business_hours`; save success refreshes `ClinicOwnerReadProvider` and returns to the real directory.
    - The owner identity is read-only, primary switching is disabled/deferred, associate/staff assignment is deferred, and unsaved identity is neutral rather than a fabricated clinic number. Targeted mock clinic/platform/tenant/settings/audit/local-storage side effects were removed from this workflow.
    - Add/Edit quota presentation uses normalized provider data only as UX. The deployed RPC remains final authority for quota, subscription, member, identity, audit, and lifecycle decisions. Activate/deactivate/archive/restore/delete/set-primary remain unavailable.
    - Focused adapter, stepper, directory, and existing owner-read regression tests are added/run locally; production build and diff check are required before browser validation.
    - Third Preview validation passed: exact Testing Clinic/Draft action-menu routes, side-preview route selection, active-only count `2`, total `3`, quota `3 / 3`, Add Branch quota UX, deep-link/hard-refresh routing, and logout protection.
...
70. **Phase 2E.3C.2A Secure Associate Dentist Provisioning / Edit Backend Contract (August 31, 2026)**:
    - Added tracked local migration `20260831133106_associate_dentist_provisioning_edit_backend_contract.sql`, a server-only Associate provisioning ledger, strict private validation/owner/quota/clinic helpers, service-role-only create/retry/delivery/edit RPCs, generated `DEN-` identity, audit events, and direct-write RLS hardening for memberships, Associate profiles, and clinic assignments.
    - Added authenticated `provision-associate-dentist` Edge Function. It fixes the role to `associate`, derives the actor from verified auth, accepts no subscriber/role/password/authority fields, creates/compensates Auth users server-side, and returns only safe provisioning/delivery state. Existing generic `provision-member-account` remains untouched and unwired for this Associate flow.
    - Initial credentials are cryptographically generated server-side, delivered only through the existing server-side registration email gateway, never stored/logged/returned, and rotate on ledger-backed delivery retry. Existing global Auth emails fail closed as `ASSOCIATE_EMAIL_UNAVAILABLE`; no arbitrary account linking occurs.
    - `complete-initial-password` now supports an active password-gated Associate membership as well as Clinic Owner, preserving the first-login RLS gate. The Phase 2E.3C.1 read baseline remains intact; its former disabled Add/Edit controls are superseded by Phase 2E.3C.3A below.
    - Local validation passed: migration reset/application, 37 pgTAP contract/RLS/audit tests, an actual two-session final-slot quota race, 34 Edge/first-login Deno tests, focused 2E.3C.1 plus branch regressions (58 tests), local Edge boot with anonymous HTTP 401, schema lint with no new warnings, and `npm run build`.
    - Remote deployment applied exactly migration `20260831133106`; deployed JWT-protected `provision-associate-dentist` version 1 and updated JWT-protected `complete-initial-password` version 2. Remote SQL/advisor inspection confirmed service-only RPC grants, owner/first-login/quota/clinic/edit/audit invariants, closed provisioning-ledger RLS, hardened Platform-only writes, preserved tenant reads, and no security-advisor error.
    - Remote anonymous provisioning returned HTTP 401. No successful provisioning, credential email, Associate Auth user, membership, profile, or clinic assignment was created. Existing tenant inventory remains one subscriber, three clinics, Plus Associate limit 6, and zero Associate usage.
...
71. **Phase 2E.3C.3A Associate Dentist Add / Edit Frontend Real-Data Cutover (August 31, 2026)**:
    - Wired Add to authenticated `provision-associate-dentist` and Edit to the local authenticated `update-associate-dentist` boundary for the existing service-role-only edit RPC. Both browser contracts use strict allowlists; protected tenant, role, identity, lifecycle, number, and password fields are excluded.
    - The form uses real active Clinic Owner provider clinics and UUID assignments, provider quota UX, immutable Edit email, authoritative refreshes, controlled safe errors, and no mock/localStorage/account/password fallback. Lifecycle controls remain disabled.
    - Local verification passed: 70 focused Clinic Owner/branch regression tests, four new Edit-boundary Deno tests, and `npm run build`. No migration/RLS/RPC/remote deployment or tenant mutation occurred.
