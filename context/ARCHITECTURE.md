...
- Remote development validation confirmed exact RPC/helper grants, empty search paths, owner read preservation/direct-write denial, Platform Admin write preservation, safe create/update DTOs and errors, server-derived identity/audit authority, quota variants, and cross-tenant concealment. Isolated fixtures ran inside a rolled-back transaction and left no rows.
- Remote concurrency and late-failure injection were intentionally not run because they require separate privileged connections or transient schema instrumentation; the passed local two-connection and rollback proofs remain recorded.
- **PHASE 2E.3B.2A = BACKEND CONTRACT REMOTELY DEPLOYED / REMOTE CONTRACT VALIDATED / FRONTEND NOT WIRED / READY FOR CHECKPOINT.** Phase 2E.3B.2 is not complete.

## Phase 2E.3B.2B Clinic Owner Add/Edit Branch Frontend Cutover - August 31, 2026

- The existing `/clinic/branches`, `/clinic/branches/new`, `/clinic/branches/view/:id`, and `/clinic/branches/edit/:id` route structure now uses the authenticated Clinic Owner provider plus the deployed `create_my_clinic_branch` and `update_my_clinic_branch` RPCs. The browser sends the strict editable-field allowlist and never supplies subscriber, clinic-number, status, primary, ownership, personnel, plan, quota, or audit authority.
- Exact RLS-backed clinic detail reads use only the requested clinic UUID with `clinics` and `clinic_business_hours`; all seven named UI weekdays map explicitly to Postgres Sunday=0 through Saturday=6. Provider refresh follows each successful create/update instead of local optimistic quota or clinic mutation.
- The approved stepper remains in place. Owner, primary-branch, associate, and staff controls are visibly read-only/deferred because the deployed contract does not persist them. Unsaved clinic identity is neutral (`Generated on save`), and no mock settings/audit/local-storage side effect is used.
- Third Preview validation passed: Testing Clinic and Draft View/Edit, selected side-preview routes, protected deep links/hard refreshes, logout clearing, active-only count `2`, total `3`, quota `3 / 3`, and quota-boundary Add Branch behavior all use authenticated provider/RLS authority. Persisted lifecycle controls remain disabled/read-only; no lifecycle, primary, or fixture data changed.
- Phase 2E.3B.2A remains the verified unchanged backend contract. Zero-hours legacy branch behavior is regression-tested but was not destructively reproduced in the linked tenant because all current clinics have seven persisted business-hour rows. **PHASE 2E.3B.2B = CLINIC BRANCH ADD/EDIT FRONTEND REAL-DATA CUTOVER / LIVE BROWSER VALIDATED / READY FOR VERIFIED CHECKPOINT.**

## Phase 2E.3C.1 Associate Dentist Directory / Detail Read Boundary - August 31, 2026

- `clinicOwnerAssociateApi.ts` derives the only tenant scope from the authenticated `ClinicOwnerReadProvider` bootstrap. It performs two bounded RLS-backed directory reads: current-subscriber Associate memberships with safe profile/professional fields, then their same-subscriber clinic assignments and clinics.
- `/clinic/dentists/view/:id` treats `:id` as the exact `subscriber_memberships.id`; its detail read requires that ID, `role = associate`, and the membership-derived subscriber before mapping only same-subscriber clinic assignments. RLS-hidden or absent rows become a safe not-found state, never a first-record/cache/mock fallback.
- The directory summary derives loaded membership statuses and normalized provider associate quota only. Add, Edit, bulk, and lifecycle actions are disabled or controlled unavailable because secure Associate provisioning and mutation contracts are deferred; no mock service, browser storage, password, or credential flow is authoritative in this runtime path.
- Live browser validation passed against the real empty tenant: zero Associate profiles/clinic assignments, no sample rows, zero summary counts, and provider-authoritative Plus quota `0 / 6`. Directory hard refresh and logout-to-`/login` protection passed without app-origin or Vercel runtime errors.
- Exact Associate detail/routing/assignment behavior remains focused automated-regression coverage because the current authenticated tenant has no Associate Dentist records or clinic assignments. No artificial live fixture was created.
- **PHASE 2E.3C.1 / ASSOCIATE DENTIST DIRECTORY / DETAIL REAL-DATA READ CUTOVER / LIVE BROWSER VALIDATED / READY FOR VERIFIED CHECKPOINT.** No migration, RLS, RPC, Edge Function, deployment, or remote data change was made.

## Phase 2E.3C.2A Secure Associate Dentist Provisioning / Edit Backend Contract - August 31, 2026

- The local-only write boundary is `verified Clinic Owner -> Associate-specific Edge Function -> service_role-only RPC ledger -> Auth + membership/profile/professional profile/clinic assignments + audit -> server email gateway`. The browser supplies only the approved personal/professional fields and clinic UUIDs; subscriber, role, actor, account state, personnel number, lifecycle, password, and audit identity are server-owned.
- `begin_associate_provisioning` derives one active `clinic_owner` membership, enforces `must_change_password = false`, validates active same-subscriber clinics, locks the subscriber row, resolves the one eligible subscription/plan, and counts active Associate memberships against the persisted `plans.limits` entry. `complete_associate_provisioning` repeats the locked final quota check before creating the active, password-gated Associate membership.
- The server generates the `DEN-` Associate number and a cryptographically generated temporary credential. Credential plaintext exists only in memory while the server sends the initial email; it is neither persisted, logged, nor included in API/audit responses. Delivery failure remains in the server-only ledger and a retry rotates the credential rather than creating another Associate.
- Direct Clinic Owner writes to memberships, Associate profiles, and clinic assignments are removed; owner reads and Platform Admin/service-role behavior remain. The later Edit boundary resolves an exact same-subscriber Associate membership, preserves immutable email/Auth/tenant/role/lifecycle identity, and validates replacement clinic sets before changing assignments.
- `complete-initial-password` now recognizes the same active password-gated Associate membership model as Clinic Owner without changing any Clinic Owner frontend routing. The former disabled Associate Add/Edit UI is superseded by the separately tracked local Phase 2E.3C.3A cutover below.
- Remote verification confirmed the tracked migration, service-only grants, hardened Platform-only write policies, JWT-protected Associate provisioning function, updated first-login function, safe anonymous 401 behavior, and unchanged tenant inventory (`0` Associate memberships/profiles/assignments on Plus `0 / 6`).
...
- `/clinic/dentists/new` now invokes only `provision-associate-dentist` with the verified personal/professional/clinic-UUID allowlist. `/clinic/dentists/edit/:membershipId` loads the exact RLS-visible `subscriber_memberships.id`, keeps email immutable, and invokes a new local authenticated `update-associate-dentist` Edge boundary that delegates to the existing service-role-only `update_my_associate_dentist` RPC.
- Clinic choices and quota UX are derived from `ClinicOwnerReadProvider`; no subscriber, role, password, generated Associate number, lifecycle state, or browser storage becomes authority. Success refreshes the provider before route transition; failure keeps form state and exposes only safe mapped messages.
- The local Edit boundary resolves the previously proven frontend-callability gap without changing migration `20260831133106`, RLS, or the existing RPC contract. It is not deployed yet. Lifecycle actions remain visibly unavailable, and no live Associate/Auth/assignment/email was created.
