...
- `complete-initial-password` now recognizes the same active password-gated Associate membership model as Clinic Owner without changing any Clinic Owner frontend routing. The current Associate directory/Add/Edit UI remains unavailable for writes until remote backend deployment and verification.
