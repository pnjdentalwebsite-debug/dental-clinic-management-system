...
- The second live-validation repair selects the exact action-menu row before navigation and routes View/Edit using that row's explicit UUID rather than a selected/primary fallback. Existing-branch lifecycle controls are visibly disabled with no callback; only new-branch draft creation remains available.
- Zero-hours legacy detail behavior remains regression-tested but was not destructively reproduced in the linked tenant because all current clinics have seven persisted business-hour rows. **PHASE 2E.3B.2B = SECOND LIVE BROWSER VALIDATION BLOCKERS FIXED LOCALLY / LIVE BROWSER RE-VALIDATION REQUIRED.**
