...
- The local validation repair separates active-only dashboard counts from quota-consuming clinic usage (`draft`, `pending`, `active`, `inactive`; not `archived`). Exact branch detail reads now treat zero persisted hours as a valid legacy profile, while partial hours and actual request failures still fail closed. Directory operating-hours summaries remain `Unavailable` by design because the directory DTO does not load hours and avoids per-row queries.
- Activate, deactivate, archive, restore, delete, and set-primary remain deferred to the branch lifecycle phase. **PHASE 2E.3B.2B = ADD/EDIT BRANCH FRONTEND REAL-DATA CUTOVER LOCALLY IMPLEMENTED / LIVE BROWSER VALIDATION IN PROGRESS.**
