...
5. Migration `20260831011234` is the latest linked-development migration. Remote catalog checks confirmed one copy of each RPC/helper, authenticated-only public RPC execution, private-helper protection, empty RPC search paths, intended RLS policies, and no authenticated/anonymous TRUNCATE privilege.
6. Isolated remote fixtures ran inside one transaction and were rolled back. Authority failures, owner reads/direct-write denial, Platform Admin writes, create/update, generated identity, seven-day hours, quota variants, tenant isolation, audit authority, and stable errors passed; a residue scan returned zero across all fixture tables.
7. Authenticated read-only smoke validation retained Angelo Dental Clinic as one active primary clinic with one active subscription and the Plus plan at `1 / 3`. No real tenant record was edited.
...
5. The third Preview validation passed: Testing Clinic action-menu View/Edit opened Testing Clinic, Draft View/Edit opened the Draft branch, and side-preview View/Edit followed the currently selected branch. Existing-branch Save Draft, Activate, and Deactivate remained visibly disabled/read-only without lifecycle callbacks.
...
## Phase 2E.3C.1 Live Associate Dentist Read Validation - August 31, 2026
...
5. Live browser validation passed with a legitimate zero-Associate/zero-assignment tenant: `/clinic/dentists` showed its truthful empty state, no mock/sample row, summary counts `0`, and the authenticated Plus quota `0 / 6`. Hard refresh, disabled Add/Edit/lifecycle controls, unreachable credential/password flows, logout-to-`/login`, and no app-origin/Vercel runtime errors all passed without remote data mutation.
6. Exact Associate detail/routing/assignment behavior—including detail hard refresh and logout protection—remains focused automated-regression coverage because the authenticated tenant has no Associate Dentist records or clinic assignments. No artificial live fixture was created.
7. **PHASE 2E.3C.1 / ASSOCIATE DENTIST DIRECTORY / DETAIL REAL-DATA READ CUTOVER / LIVE BROWSER VALIDATED / READY FOR VERIFIED CHECKPOINT.**

## Phase 2E.3C.2A Local Associate Provisioning / Edit Contract Validation - August 31, 2026

1. Apply the ordered migration only to a local/test Supabase environment. Confirm `associate_provisioning_attempts` has RLS enabled with no browser policies and that the new public Associate RPCs are executable only by `service_role`.
2. Run the pgTAP contract suite and the two-session concurrency script. Confirm exact owner resolution, completed-first-login enforcement, strict field rejection, plan-derived quota enforcement, same-subscriber active-clinic validation, atomic profile/membership/assignment writes, generated `DEN-` identity, audit hygiene, edit identity protection, and direct-browser-write denial.
...
5. The browser Add/Edit flow remains controlled unavailable. Do not wire `AssociateDentistsPage`, `AssociateDentistFormPage`, or `App.tsx` until the migration and Edge Function are deliberately deployed and remote backend validation succeeds.
6. **PHASE 2E.3C.1 = COMPLETE / VERIFIED CHECKPOINT. PHASE 2E.3C.2A = SECURE ASSOCIATE DENTIST PROVISIONING / EDIT BACKEND CONTRACT / LOCALLY IMPLEMENTED / REMOTE DEPLOYMENT / VERIFICATION REQUIRED.**
